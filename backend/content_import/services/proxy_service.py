...
from ..utils.image_resolution import parse_srcset
...
class ProxyService:
    ...
    def fetch_and_rewrite(
        ...
        try:
            ...
            # Add resolution indicator script for high-res images
            self._inject_resolution_indicator_script(soup)
    ...
    def _rewrite_urls(self, soup: BeautifulSoup, base_url: str, request=None):
        ...
        for img in soup.find_all("img"):
            # Extract srcset data if present
            srcset_metadata = None
            if img.get("srcset"):
                # Parse srcset and make URLs absolute
                srcset_data = parse_srcset(img["srcset"])
                srcset_metadata = []
                for item in srcset_data:
                    absolute_url = urljoin(base_url, item["url"])
                    srcset_metadata.append(
                        {"url": absolute_url, "descriptor": item["descriptor"]}
                    )

            # Rewrite src with srcset metadata
            if img.get("src"):
                absolute_url = urljoin(base_url, img["src"])
                img["src"] = self._create_proxy_url(
                    absolute_url, request, srcset_metadata
                )

            # Rewrite srcset URLs (keep srcset for browser-side rendering)
            if img.get("srcset"):
                srcset_parts = []
    ...
    def _create_proxy_url(self, url: str, request=None, srcset_metadata=None) -> str:
        """
        ...
        Args:
            url: Original URL
            request: Django request object for building absolute URLs
            srcset_metadata: Optional srcset data for high-resolution detection
        ...
        # Generate signed token for the asset URL with optional srcset metadata
        metadata = None
        if srcset_metadata:
            metadata = {"srcset": srcset_metadata}

        signed_token = sign_proxy_token(url, metadata)
    ...
    def _inject_resolution_indicator_script(self, soup: BeautifulSoup):
        """
        Inject JavaScript to detect and display resolution badges on high-res images.

        Args:
            soup: BeautifulSoup object
        """
        # Add CSS for resolution badges
        style = soup.new_tag("style")
        style.string = """
        .resolution-badge {
            position: absolute;
        ...
        .resolution-badge.res-3x {
            background: rgba(22, 163, 74, 0.95);
        }
        .resolution-badge.res-2x {
            background: rgba(37, 99, 235, 0.95);
        }
        ...
        # Add JavaScript for resolution detection
        script = soup.new_tag("script")
        script.string = """
        (function() {
            // Resolution indicator for high-res images
            function addResolutionIndicators() {
                const images = document.querySelectorAll('img');
                
                images.forEach((img) => {
                    // Skip if already wrapped
                    if (img.parentElement.classList.contains('resolution-badge-wrapper')) {
                        return;
                    }
                    
                    // Intercept the image load to read headers
                    const src = img.src;
                    if (!src || !src.includes('proxy-asset')) {
                        return; // Only process proxied images
                    }
                    
                    // Fetch with HEAD to get headers without downloading again
                    fetch(src, { method: 'HEAD' })
                        .then(response => {
                            ...
                            if (resolution && resolution !== '1x') {
                                ...
                                wrapper.className = 'resolution-badge-wrapper';
                                ...
                                // Add resolution class for color
                                if (resolution.startsWith('3')) {
                                    badge.classList.add('res-3x');
                                } else if (resolution.startsWith('2')) {
                                    badge.classList.add('res-2x');
                                }
                                ...
                                if (source === 'srcset') {
                                    icon = '📱 ';
                                } else if (source && source.includes('url-pattern')) {
                                    icon = '🔍 ';
                                }
                                
                                badge.textContent = icon + resolution;
                                badge.title = `High-resolution image: ${resolution} (${source || 'detected'})${dimensions ? '\\nDimensions: ' + dimensions : ''}`;
                                
                                wrapper.appendChild(badge);
                                
                                console.log(`🖼️  High-res image detected: ${resolution} from ${source}`);
                            }
                        })
                        .catch(err => {
                            console.debug('Failed to check image resolution:', err);
                        });
                });
            }
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', addResolutionIndicators);
            } else {
                addResolutionIndicators();
            }
            
            // Also run after a short delay to catch lazy-loaded images
            setTimeout(addResolutionIndicators, 1000);
            ...
            const observer = new MutationObserver((mutations) => {
                ...
                if (hasNewImages) {
                    setTimeout(addResolutionIndicators, 100);
                }
            });

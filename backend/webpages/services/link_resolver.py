...
def is_link_object(value: Any) -> bool:
    ...
    return link_type in ("internal", "external", "email", "phone", "anchor")
...
def parse_link_string(value: str) -> Optional[Dict[str, Any]]:
    ...
    # Quick check - link objects start with {
    value = value.strip()
...
def resolve_links_in_config(
    config: Dict[str, Any], 
    request=None,
    url_fields: tuple = ("url", "href", "link")
) -> Dict[str, Any]:
    """
    ...
    for key, value in config.items():
        ...
        if key in url_fields:
            if is_link_object(value) or (isinstance(value, str) and parse_link_string(value)):

...
class PageDebugExportTests(APITestCase):
    def setUp(self):
        ...
        self.import_log = ImportLog.objects.create(
            source_url="https://example.com/source?token=secret-token#private",
            slot_name="main",
    ...
    def test_debug_export_allows_authenticated_editors(self):
        ...
        self.assertEqual(response.data["generated_by"]["username"], "debug-editor")
        self.assertEqual(response.data["versions"][0]["widgets"], self.version.widgets)
        self.assertEqual(
            response.data["versions"][0]["widget_summary"]["top_level_widget_count"],
            1,
        )

...
def detect_parent_theme(
    theme_data: Dict[str, Any], all_themes: Dict[str, Dict]
) -> Optional[str]:
    """
...
def generate_theme_from_json(
            ...
            if len(parts) >= 3:
                ...
                try:
                    ...
                    if themes_idx + 2 < len(parts):
                        ...
                        parent_theme_dir = (
                            theme_dir.parent.parent
                            / category
                            / parent_name.lower().replace(" ", "_")
                        )
                        ...
                        parent_class = (
                            "".join(word.capitalize() for word in parent_name.split())
                            + "Theme"
                        )
                    ...
                    if "dark" in theme_data.get("name", "").lower():
                        parent_import = (
                            f"themes.base.{parent_name.lower().replace(' ', '_')}.theme"
                        )
                    else:
                        parent_import = (
                            f"themes.base.{parent_name.lower().replace(' ', '_')}.theme"
    ...
    if use_modules:
        ...
        if theme_data.get("design_groups"):
            design_groups_code = generate_design_groups_module(
                theme_data["design_groups"]
            )
            (theme_dir / "design_groups.py").write_text(
                design_groups_code, encoding="utf-8"
            )

        if theme_data.get("component_styles"):
            component_styles_code = generate_component_styles_module(
                theme_data["component_styles"]
            )
            (theme_dir / "component_styles.py").write_text(
                component_styles_code, encoding="utf-8"
            )

        if theme_data.get("image_styles"):
            image_styles_code = generate_image_styles_module(theme_data["image_styles"])
            (theme_dir / "image_styles.py").write_text(
                image_styles_code, encoding="utf-8"
            )

        if theme_data.get("table_templates"):
            table_templates_code = generate_table_templates_module(
                theme_data["table_templates"]
            )
            (theme_dir / "table_templates.py").write_text(
                table_templates_code, encoding="utf-8"
            )

        if theme_data.get("breakpoints"):
            breakpoints_code = generate_breakpoints_module(theme_data["breakpoints"])
            (theme_dir / "breakpoints.py").write_text(
                breakpoints_code, encoding="utf-8"
            )
    ...
    python_code = generate_theme_class_code(
        theme_data, parent_class, parent_import, use_modules=use_modules
    )
    ...
    if (
        ...
        and ("http://" in image_url or "https://" in image_url)
    ):
        # Image download will be handled separately in sync_service
        # For now, just note that we need to download it
        pass

    # Download and save design group images
    download_design_group_images(theme_data, theme_dir)
...
def extract_design_group_image_urls(design_groups: Dict[str, Any]) -> list:
    """
    Extract all image URLs from design_groups.layoutProperties.
    Returns list of tuples: [(url, metadata), ...]
    """
    image_urls = []
                            ...
                            for image_key, image_data in props["images"].items():
                                if isinstance(image_data, dict):
                                    ...
                                    if url:
                                        ...
                                        image_urls.append((url, metadata))
    return image_urls


def download_design_group_images(theme_data: Dict[str, Any], theme_dir: Path) -> None:
    """
    Download design group images from URLs and save to local files.
    Updates design_groups JSON to reference local file paths.
    """
    import requests
    import logging
    ...
    image_urls = extract_design_group_image_urls(design_groups)

    if not image_urls:
        return

    # Create design_group_images directory
    images_dir = theme_dir / "design_group_images"
    images_dir.mkdir(exist_ok=True)

    # Download each image
    for url, metadata in image_urls:
        try:
            ...
            logger.info(f"Downloaded design group image: {filename}")

        except Exception as e:
            logger.warning(f"Failed to download design group image {url}: {e}")

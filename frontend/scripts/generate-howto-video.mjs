...
const recordedAudioPathFromUrl = (url = '') => {
  ...
  if (blockAudioIndex >= 0) {
    ...
    const validName = /^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(fileName || '')
    if (!['recorded', 'elevenlabs'].includes(kind) || !validName || parts.length !== blockAudioIndex + 3) return ''
  ...
  const validId = /^[A-Za-z0-9][A-Za-z0-9_-]*$/.test(id || '')
  const validName = /^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(audioName || '')
  if (!validId || action !== 'audio' || !validName || parts.length !== recordIndex + 4) return ''

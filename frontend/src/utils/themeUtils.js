        ...
        if (group.layoutProperties) {
            ...
            for (const [part, partBreakpoints] of Object.entries(group.layoutProperties)) {
                ...
                for (const bpKey of ['xs', 'sm', 'md', 'lg', 'xl']) {
                    ...
                    if (bpKey === 'xs') {
                        // xs is the base - merge legacy formats
                        if (partBreakpoints.default) bpProps = { ...bpProps, ...partBreakpoints.default };
                        if (partBreakpoints.mobile) bpProps = { ...bpProps, ...partBreakpoints.mobile };
                        if (partBreakpoints.xs) bpProps = { ...bpProps, ...partBreakpoints.xs };
                    } else if (bpKey === 'sm') {
                        // sm was previously base, now check for both sm and legacy desktop
                    ...
                    for (const [prop, value] of Object.entries(bpProps)) {
                        ...
                        if (prop === 'backgroundImage' && value && typeof value === 'object' && value.url) {
                            // NOTE: This is only used for preview in theme editor
                            // Production CSS is generated by backend which routes through imgproxy
                            // Preview uses direct URLs for simplicity (async imgproxy calls would complicate preview)

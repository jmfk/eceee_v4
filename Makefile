...
HELP := $(filter help --help -h,$(MAKECMDGOALS))
...
# Dummy targets for help flags (prevents errors if passed as goals)
--help:
	@:
-h:
	@:
...
# Help target with support for specific commands
...
ifneq ($(HELP),)
	$(call print_help,use-external-infra)
else
